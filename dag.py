...
# 5 datanodes × 16GB RAM × 16 cores (per node: 13.3GB containers, 14 vcores for YARN)
# 10 executors total: 2 per node × 5 nodes
# executor: 6G heap + 1G overhead = 7G per executor (under 13.3GB container limit)
# AQE enabled: runtime partition coalescing, skew join handling, automated tune
SPARK_COMMON = """
        --master yarn \\
        --deploy-mode client \\
        --driver-memory 2G \\
...
with DAG(
    dag_id="spark_minio_transform_pipeline",
...
"""
    ...
    read_bronze >> bronze_to_silver >> silver_to_gold
