...
from dags.combined_domains.foxai_config import DOMAIN_REGISTRY_FILE, MINIO_ACCESS_KEY, MINIO_ENDPOINT, MINIO_SECRET_KEY
...
def process_taxi_domain(domain_name: str, domain_cfg: dict):
    ...
    if yellow_paths:
        ...
        if branch_dfs:
            ...
            yellow_table = f"silver_catalog.{silver_namespace}.{datasets.get('yellow', {}).get('silver_table', 'yellow_taxi')}"
            yellow_silver.writeTo(yellow_table).createOrReplace()
            logger.info("Wrote %s", yellow_table)
    ...
    if green_paths:
        ...
        green_silver = green_raw.select(
         ...
         .withColumn("silver_processed_ts", current_timestamp())
        green_table = f"silver_catalog.{silver_namespace}.{datasets.get('green', {}).get('silver_table', 'green_taxi')}"
        green_silver.writeTo(green_table).createOrReplace()
        logger.info("Wrote %s", green_table)
    ...
    if fhv_paths:
        ...
        fhv_silver = fhv_raw.select(
         ...
         .withColumn("silver_processed_ts", current_timestamp())
        fhv_table = f"silver_catalog.{silver_namespace}.{datasets.get('fhv', {}).get('silver_table', 'fhv_trip')}"
        fhv_silver.writeTo(fhv_table).createOrReplace()
        logger.info("Wrote %s", fhv_table)
    ...
    if fhvhv_paths:
        ...
        fhvhv_silver = fhvhv_raw.select(
         ...
         .withColumn("silver_processed_ts", current_timestamp())
        fhvhv_table = f"silver_catalog.{silver_namespace}.{datasets.get('fhvhv', {}).get('silver_table', 'fhvhv_trip')}"
        fhvhv_silver.writeTo(fhvhv_table).createOrReplace()
        logger.info("Wrote %s", fhvhv_table)
...
def process_non_taxi_domain(domain_name: str, domain_cfg: dict):
    ...
    for topic_name, topic_cfg in topics.items():
        ...
        silver_table_fqn = f"silver_catalog.{silver_namespace}.{silver_table}"
        silver_df.writeTo(silver_table_fqn).createOrReplace()
        logger.info("Wrote %s", silver_table_fqn)
