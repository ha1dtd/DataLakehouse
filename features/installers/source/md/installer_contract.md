# Lakehouse Installer Contract

## Purpose

The Lakehouse installer exists to turn customer Linux servers into a working Lakehouse platform cluster so the remaining customer work is:
...
```bash
cd features/installers/source
GOOS=linux GOARCH=amd64 go build -o ../../../builds/foxai-installer-linux-amd64 .
```

## Source Of Truth

- Active module source: `features/installers/source/`
- Legacy shell/bootstrap references: `features/installers/script/`
- Output binary: `builds/foxai-installer-linux-amd64`

## Installer Goal

On success, the installer should leave the cluster in a state where:

- NameNode and DataNodes are configured for the Lakehouse platform
...
- `install`
  - create or converge a Lakehouse cluster shape
...
- `reconcile`
  - align an existing cluster to the requested Lakehouse shape
- `dry-run`
...
- `fresh`
  - no prior Lakehouse/HDFS DataNode state detected
- `compatible`
...
- `partial`
  - host contains incomplete or broken Lakehouse-managed state
- `unreadable`
...
Lakehouse owns the platform configuration it needs to make Hadoop, Spark, and cluster wiring work.

Practical ownership model:

- full ownership:
  - Lakehouse-managed Hadoop config under the installer-managed Hadoop home
...
- online install path
- apt-based distro
- Lakehouse-managed cluster paths
- fresh servers or servers where Lakehouse is taking ownership of its own platform paths
