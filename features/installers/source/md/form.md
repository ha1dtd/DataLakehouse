# Lakehouse Installer Product Form
...
10. Should `reconcile` be allowed to overwrite drifted Lakehouse-managed files? `yes/no` yes, but it should tell the user what files got overwrited

## 3. Ownership

11. Which paths does Lakehouse fully own?
12. Which paths may Lakehouse edit only inside managed blocks?
13. Which paths must Lakehouse never touch automatically?
14. Does Lakehouse own `/opt/spark`? `yes/no/depends`
15. Does Lakehouse own `/home/ubuntu/hadoop`? `yes/no/depends`
16. Does Lakehouse own `/etc/hosts` full file or only a Lakehouse block?
17. Does Lakehouse own user `.bashrc` fully or only append/update a managed block?
...
19. If a config differs from desired Lakehouse value, should installer:
