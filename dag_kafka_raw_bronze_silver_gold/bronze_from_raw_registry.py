...
ERROR_PREFIX = os.environ.get("ERROR_PREFIX", "bronze_kafka")

spark = SparkSession.builder \
    .appName("BronzeFromRawRegistry") \
...
spark.sql("CREATE NAMESPACE IF NOT EXISTS bronze_catalog.default")
spark.sql(
    """
    CREATE TABLE IF NOT EXISTS bronze_catalog.default.raw_file_registry (
        job_id STRING,
        source_name STRING,
        source_uri STRING,
        raw_bucket STRING,
...
raw_df = spark.read.table("raw_catalog.default.raw_registry") \
    .filter((col("status") == "landed") & (col("file_type").isin("json", "csv", "xml")))
...
for row in raw_df.collect():
    ...
    bronze_object_key = f"raw/{base_name}.parquet"
    bronze_uri = f"s3a://bronze/{bronze_object_key}"
    status = "success"
    error_message = None
    ...
    try:
        ...
        try:
            ...
            err_path = jvm.org.apache.hadoop.fs.Path(f"s3a://{ERROR_BUCKET}/{ERROR_PREFIX}/{row['file_name']}")
...
if records:
    out = spark.createDataFrame(records, schema="job_id string, source_name string, source_uri string, raw_bucket string, raw_object_key string, bronze_object_key string, file_name string, file_type string, bronze_format string, status string, error_message string, processed_ts timestamp")
    out.writeTo("bronze_catalog.default.raw_file_registry").append()
