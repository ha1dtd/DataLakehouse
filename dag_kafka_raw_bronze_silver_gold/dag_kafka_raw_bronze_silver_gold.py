...
with DAG(
    dag_id="kafka_raw_bronze_silver_gold_pipeline",
    ...
    enqueue_ingest_requests = BashOperator(
        ...
        export INGEST_SOURCES_FILE={SCRIPT_BASE}/ingest_sources_demo.json
        python3 {SCRIPT_BASE}/kafka_enqueue_ingest.py
"""
    )
    ...
    kafka_consume_to_raw = BashOperator(
        ...
        {SCRIPT_BASE}/kafka_consume_to_raw.py
"""
    )

    bronze_from_raw_registry = BashOperator(
        task_id="bronze_from_raw_registry",
        bash_command=f"""
        /opt/spark/bin/spark-submit \
        {SPARK_COMMON} \
        ...
        {S3A_CONF} \
        {SCRIPT_BASE}/bronze_from_raw_registry.py
"""
    )

    silver_from_bronze_registry = BashOperator(
        task_id="silver_from_bronze_registry",
        bash_command=f"""
        /opt/spark/bin/spark-submit \
        {SPARK_COMMON} \
        ...
        {S3A_CONF} \
        {SCRIPT_BASE}/silver_from_bronze_registry.py
"""
    )

    gold_from_ingested_structured = BashOperator(
        task_id="gold_from_ingested_structured",
        bash_command=f"""
        /opt/spark/bin/spark-submit \
        {SPARK_COMMON} \
        --packages org.apache.iceberg:iceberg-spark-runtime-3.5_2.12:1.4.3,org.apache.hadoop:hadoop-aws:3.3.4,com.amazonaws:aws-java-sdk-bundle:1.12.262 \
        ...
        {S3A_CONF} \
        {SCRIPT_BASE}/gold_from_ingested_structured.py
"""
    )

    enqueue_ingest_requests >> kafka_consume_to_raw >> bronze_from_raw_registry >> silver_from_bronze_registry >> gold_from_ingested_structured
