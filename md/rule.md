...
- If resuming an in-progress task or any task that may have been compacted, read files in this exact order:
  ...
  3. `md/logs.md`
  4. `md/progress.md`
- If user only says "read rule.md" outside of session start, read only `md/rule.md` — skip the other three.
...
- The session state file is exactly `md/progress.md`.
...
- Create or update `md/progress.md` with:
  - Task goal
  - Files in scope
...
- Immediately update `md/progress.md` with:
  - File changed
  - Exact change made
  - Verification status
  - Next file in scope
...
- Stop editing
- Update `md/progress.md`
- Update `logs.md` if progress is meaningful
- Only then continue

### On resume or after any compaction

- Read `md/rule.md` → `md/project.md` → `md/logs.md` → `md/progress.md`
...
- Source of truth priority:
  ...
  3. repo memory
  4. chat history last
- If `md/progress.md` conflicts with chat memory, trust `md/progress.md` until on-disk files are re-read.
