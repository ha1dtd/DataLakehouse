...
- Current implementation checkpoint:
  - active installer source tree now lives at `features/installers/source/`
  ...
  - premise-specific notes now exist at `features/installers/script/foxai_installer_premise_notes.md`
  - source-of-truth setup logic remains `features/installers/script/setup_namenode_v5.sh` and `features/installers/script/setup_datanode.sh`
  - dedicated product buckets now exist at:
    - `features/addon/`
    - `features/licensing/`
- Current packaging boundary:
  - include platform installation, configuration, and cluster bootstrap
  - exclude platform demo DAGs, platform demo job scripts, customer DAGs, and customer job scripts
...
- Validate the active installer flow in `features/installers/source/` against the tested shell/bootstrap references in `features/installers/script/`
- Audit and harden the active Go installer in `features/installers/source/` before broadening compatibility beyond the current baseline
...
- Turn `features/addon/` into the real add-on contract area:
  - extension folder structure
  - sample add-on template
  - loading / ownership / failure-boundary rules
...
- Licensing phase remains later and generic:
  ...
  - expected implementation area is `features/licensing/`
...
- Packaging-first product direction clarified and started:
  ...
  - one unified installer entrypoint created at `features/installers/script/foxai_installer.sh`
  - premise-specific rollout notes recorded at `features/installers/script/foxai_installer_premise_notes.md`
...
- Validate the unified installer `features/installers/script/foxai_installer.sh` against the current tested deployment flow
